 ...
 */
   ...
   */
  function MGRStoLLbounding(mgrs) {
   ...
   * @public
   * @param {number} rad the angle in radians.
   * @return {number} the angle inMils.
   */
  function radToMils(rad) {
  ...
  function encode(utm, accuracy) {
   ...
   /**
   * Encodes a UTM location as MGRS Object.
   *
   * @private
   * @param {object} utm An object literal with easting, northing,
   *     zoneLetter, zoneNumber
   * @param {number} accuracy Accuracy in digits (0-5).
   * @return {string} An object literal with zoneNumber, zoneLetter,
   *     easting, northing, 100kID.
   */
  function UTMtoMGRS(utm, accuracy) {
    ...
    return {
      ...
      easting,
      northing,
      mgrsString: utm.zoneNumber+utm.zoneLetter+id100k+" "+easting+" "+northing
    };
  }
...
northing meters ${mgrsString}`);
  ...
  function MGRStoUTM(mgrsString) {
    return decode(mgrsString);
  }
