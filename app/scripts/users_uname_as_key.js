...
async function CreateUser(fname, lname, password, level, uname) {
    ...
    return { fname, level, password, lname, password, authKey, status: true }
}
...
async function create(fname, lname, password, level, userName) {
    ...
    if (nameExists) {
        ...
        return { error: true, msg: "User Name Exists", data: null };
    } else {
...
async function getAllUsers() {
    let loaded = await getAll(["fname", "lname", "uname", "level"])
    if (!loaded.error) {
        console.log("getAllUsers",loaded.data)
    }
    return loaded;
}
...
async function getAllUsernames() {
    ...
    if (!loaded.error) {
        console.log("getAllUsernames",loaded.data)
    }
}
